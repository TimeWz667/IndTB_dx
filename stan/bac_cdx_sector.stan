data {
  ...
  real<lower=0.05, upper=1> p_csi_pub;
...
transformed parameters {
  ...
  // Entry points
  r_tb_pub = r_test * p_tb * p_csi_pub;
  r_tb_eng = r_test * p_tb * (1 - p_csi_pub) * p_csi_ppm * rp_ent;
  r_tb_pri = r_test * p_tb * (1 - p_csi_pub) * (1 - p_csi_ppm) * rp_ent;
  
  r_nontb_pub = r_test * (1 - p_tb) * p_csi_pub;
  r_nontb_eng = r_test * (1 - p_tb) * (1 - p_csi_pub) * p_csi_ppm * rp_ent;
  r_nontb_pri = r_test * (1 - p_tb) * (1 - p_csi_pub) * (1 - p_csi_ppm) * rp_ent;
  ...
  p_pub = r_tb_pub * (p_tp_bac_pub + p_tp_cdx_pub);
  p_pub = p_pub / (p_pub + r_tb_eng * (p_tp_bac_eng + p_tp_cdx_eng) + r_tb_pri * p_tp_cdx_pri);
...
generated quantities {
  ...
  ppv_pub = r_tb_pub * (p_tp_bac_pub + p_tp_cdx_pub) / r_det_pub;
  ppv_eng = r_tb_eng * (p_tp_bac_eng + p_tp_cdx_eng) / r_det_eng;
  ppv_pri = r_tb_pri * p_tp_cdx_pri / r_det_cdx_pri;
