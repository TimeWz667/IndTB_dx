...
ll <- list(
  Perfect = c(
    ...
    NoRelapse = "No Relpase after treatment"
  ),
  Delayed = c(
    Baseline = "Baseline",
...
## Loading data -----
for (suffix in c("Perfect", "Delayed")) {
  scs <- ll[[suffix]]
  suffix <- glue::as_glue(suffix)
  
  sim <- read_csv(here::here("out", "post_dyage", "Sim_Intv" + suffix + ".csv"))
  ...
  g_avt
  ggsave(g_avt, filename = here::here("docs", "figs", "g_intv_" + suffix + "_avt.png"), width = 12, height = 5)
